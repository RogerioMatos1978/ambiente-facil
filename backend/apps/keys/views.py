...
class ChaveViewSet(
    ...
    def devolver(self, request, ambiente_id=None):
        """Marca a chave como devolvida à guarita ao fim do uso."""
        chave = self.get_object()
        if chave.status != StatusChave.OCUPADA:
            raise DRFValidationError({"detalhes": "Esta chave não está retirada no momento."})

        chave.status = StatusChave.DEVOLVIDA
        chave.devolvida_em = timezone.now()
        chave.save()
        ...
        registrar(
            ...
            "Chave",
            chave.id,
            descricao=f"Chave de '{chave.ambiente.nome}' devolvida na guarita.",
            request=request,
        )
        return Response(ChaveSerializer(chave).data)
